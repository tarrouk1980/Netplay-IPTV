...
function MessagesContent() {
  ...
  const QUICK_REPLIES = [
    ...
    "Votre commande a bien été expédiée, vous recevrez votre colis sous 2-3 jours ouvrés.",
    "Ce produit est actuellement en stock et disponible à la livraison.",
    "Nous proposons la livraison sur tout le territoire tunisien. Les frais sont de 8 TND.",
    "Pour toute réclamation, n'hésitez pas à nous contacter directement ici.",
    "Merci pour votre confiance ! Votre satisfaction est notre priorité. ⭐",
  ];
        ...
        <div className="flex gap-4 h-[600px] bg-white rounded-2xl border border-slate-100 overflow-hidden" style={{ boxShadow: "0 2px 16px rgba(0,0,0,0.07)" }}>
          ...
          {activeId ? (
            <div className="flex-1 flex flex-col">
              ...
              {/* Input */}
              {showQuickReplies && (
                <div className="px-4 pb-2 border-t border-slate-50">
                  <p className="text-xs font-semibold text-slate-400 mb-1.5 pt-2">Réponses rapides :</p>
                  <div className="flex flex-wrap gap-1.5">
                    {QUICK_REPLIES.map((r, i) => (
                      <button key={i} type="button" onClick={() => { setMessage(r); setShowQuickReplies(false); }}
                        className="text-xs bg-rose-50 text-rose-800 border border-rose-200 rounded-lg px-2.5 py-1 hover:bg-rose-100 transition text-left max-w-[240px] truncate">
              ...
              <form onSubmit={sendMessage} className="p-4 border-t border-slate-100 flex gap-3">
                {user?.role === "SELLER" && (
                  <button type="button" onClick={() => setShowQuickReplies(v => !v)}
                    className={`text-sm px-3 py-2 rounded-xl transition ${showQuickReplies ? "bg-rose-100 text-rose-800" : "bg-slate-100 text-slate-500 hover:bg-slate-200"}`}
