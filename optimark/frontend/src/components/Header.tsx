...
export default function Header() {
  ...
  useEffect(() => {
    api.get("/search/trending").then(res => {
      setTrendingQueries((res.data?.data || []).map((t: any) => t.query).slice(0, 8));
    }).catch(() => {});
  }, []);
        ...
        <div className="max-w-7xl mx-auto px-4">
          ...
          <div className="flex items-center gap-3 py-3">
            ...
            <div ref={searchRef} className="hidden md:flex flex-1 max-w-2xl relative">
              <form onSubmit={handleSearch} className="flex w-full border border-slate-200 rounded-xl overflow-hidden focus-within:border-rose-800 focus-within:ring-2 focus-within:ring-rose-100 transition">
                <input
                  type="text"
                  value={search}
                  onChange={(e) => handleSearchInput(e.target.value)}
                  onFocus={() => setShowSuggestions(suggestions.length > 0 || trendingQueries.length > 0)}
              ...
              {showSuggestions && (suggestions.length > 0 || (search.trim().length < 2 && trendingQueries.length > 0)) && (
                <div className="absolute top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl z-50 overflow-hidden">
                  {search.trim().length < 2 && trendingQueries.length > 0 && (
                    <div className="px-4 pt-3 pb-2">
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-2">🔥 Tendances</p>
                  ...
                  {suggestions.map((s) => (
                    <Link key={`${s.type}-${s.id}`} href={s.href}
                      ...
                      <span className="flex-1 text-sm text-slate-800 font-medium truncate">{s.label}</span>
            ...
            <div className="hidden md:flex items-center gap-2 flex-shrink-0 ml-auto">
              ...
              {user ? (
                <div className="flex items-center gap-2">
                  {user.role === "ADMIN" && (
                    <Link href="/admin" className="text-xs font-bold bg-slate-900 text-white px-3 py-1.5 rounded-lg hover:bg-slate-700 transition">Admin</Link>
                  )}
                  <Link href="/messages" className="p-2.5 text-slate-500 hover:text-rose-800 transition" title="Messages">
