        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 mb-10">
          <div className="col-span-2 md:col-span-1">
            <p className="text-2xl font-black text-white mb-3">
              <span className="text-rose-600">OPTI</span>MARK
            </p>
            ...
            <div className="flex gap-3">
              {["f", "in", "tw"].map((s) => (
                <a key={s} href="#" className="w-9 h-9 bg-slate-800 hover:bg-rose-800 rounded-lg flex items-center justify-center text-xs font-bold text-slate-400 hover:text-white transition-all duration-200">
                  {s}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5 text-sm">
              {[["Accueil", "/"], ["Produits", "/produits"], ["Services", "/services"], ["Live", "/live"], ["Tarifs", "/pricing"]].map(([label, href]) => (
                <li key={href}><Link href={href} className="hover:text-rose-600 transition">{label}</Link></li>
              ))}
            </ul>
          </div>
