...
const QUICK_FILTERS = [
  { key: 'all', label: 'الكل' },
  { key: 'Tunisie', label: 'تونس' },
  { key: 'Maroc', label: 'المغرب' },
  { key: 'Algerie', label: 'الجزائر' },
  { key: '5stars', label: '5★' },
  { key: 'Resort', label: 'منتجع' },
];

const DESTINATIONS = [
  { city: 'تونس', cityEn: 'Tunis', country: 'تونس', image: 'https://picsum.photos/400/300?random=50' },
  { city: 'جربة', cityEn: 'Djerba', country: 'تونس', image: 'https://picsum.photos/400/300?random=51' },
  { city: 'الحمامات', cityEn: 'Hammamet', country: 'تونس', image: 'https://picsum.photos/400/300?random=52' },
  { city: 'سوسة', cityEn: 'Sousse', country: 'تونس', image: 'https://picsum.photos/400/300?random=53' },
  { city: 'مراكش', cityEn: 'Marrakech', country: 'المغرب', image: 'https://picsum.photos/400/300?random=56' },
  { city: 'الدار البيضاء', cityEn: 'Casablanca', country: 'المغرب', image: 'https://picsum.photos/400/300?random=57' },
  { city: 'الجزائر', cityEn: 'Alger', country: 'الجزائر', image: 'https://picsum.photos/400/300?random=58' },
  { city: 'وهران', cityEn: 'Oran', country: 'الجزائر', image: 'https://picsum.photos/400/300?random=59' },
];

// Currency per country
const CURRENCY_MAP = {
  تونس: { code: 'TND', label: t.common.tnd },
  المغرب: { code: 'MAD', label: t.common.mad },
  الجزائر: { code: 'DZD', label: t.common.dzd },
};

function getCurrency(country) {
  return CURRENCY_MAP[country] || CURRENCY_MAP['تونس'];
}

export default function HotelSearchScreenAR({ navigation }) {
  ...
  const [selectedCountry, setSelectedCountry] = useState('تونس');
  ...
  function formatDisplayDate(str) {
    ...
    return d.toLocaleDateString('ar-TN', { day: '2-digit', month: 'short' });
  }
  ...
  function handleQuickFilter(key) {
    setQuickFilter(key);
    let dest = '';
    if (key === 'Tunisie') { dest = 'Tunis'; setSelectedCountry('تونس'); }
    else if (key === 'Maroc') { dest = 'Marrakech'; setSelectedCountry('المغرب'); }
    else if (key === 'Algerie') { dest = 'Alger'; setSelectedCountry('الجزائر'); }
  ...
  function handleDestinationTap(dest) {
    setDestination(dest.cityEn);
    setSelectedCountry(dest.country);
    setSuggestions([]);
    navigation.navigate('HotelResults', { destination: dest.cityEn, checkIn, checkOut, guests });
  }
    ...
    <View style={{ flex: 1, backgroundColor: '#F8F9FA' }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
        ...
        >
          <View style={styles.heroHeader}>
            ...
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={[styles.heroTagline, { writingDirection: 'rtl', textAlign: 'right' }]}>✈ {t.search.subtitle}</Text>
              <Text style={[styles.heroTitle, { writingDirection: 'rtl', textAlign: 'right' }]}>{t.search.title}</Text>
            </View>
          </View>
          ...
          <View style={styles.searchCard}>
            ...
            <View style={[styles.inputGroup, { flexDirection: 'row-reverse' }]}>
              <Ionicons name="search" size={18} color="#FF6B35" style={styles.inputIconRTL} />
              <TextInput
            ...
            {suggestions.length > 0 && (
              <View style={styles.suggestions}>
                {suggestions.map((s, i) => (
                  <TouchableOpacity key={i} style={[styles.suggestionItem, { flexDirection: 'row-reverse' }]} onPress={() => { setDestination(s.label); setSuggestions([]); navigation.navigate('HotelResults', { destination: s.label, checkIn, checkOut, guests }); }}>
                    <Ionicons name={s.type === 'hotel' ? 'bed-outline' : 'location-outline'} size={16} color="#718096" />
                    <View style={{ marginRight: 8, alignItems: 'flex-end' }}>
                      <Text style={[styles.suggestionLabel, { writingDirection: 'rtl' }]}>{s.label}</Text>
                      <Text style={[styles.suggestionSub, { writingDirection: 'rtl' }]}>{s.sublabel}</Text>
                    </View>
                  </TouchableOpacity>
                ))}
              </View>
            ...
            {/* Dates Row */}
            <View style={[styles.datesRow, { flexDirection: 'row-reverse' }]}>
              <View style={styles.dateBox}>
                <Text style={[styles.dateLabel, { textAlign: 'right' }]}>{t.search.checkOut.toUpperCase()}</Text>
                <View style={[styles.dateControl, { flexDirection: 'row-reverse' }]}>
                  <TouchableOpacity onPress={() => adjustCheckOut(-1)} style={styles.dateArrow}>
                    <Ionicons name="chevron-forward" size={16} color="#004E89" />
                  </TouchableOpacity>
                  <View style={[styles.dateCenter, { flexDirection: 'row-reverse' }]}>
              ...
              <View style={styles.nightsBadge}>
                ...
                <Text style={styles.nightsLabel}>{t.common.night}</Text>
              </View>

              <View style={styles.dateBox}>
                <Text style={[styles.dateLabel, { textAlign: 'right' }]}>{t.search.checkIn.toUpperCase()}</Text>
                <View style={[styles.dateControl, { flexDirection: 'row-reverse' }]}>
            ...
            {/* Guests Row */}
            <TouchableOpacity style={[styles.guestsRow, { flexDirection: 'row-reverse' }]} onPress={() => setGuestModalVisible(true)}>
              <Ionicons name="people-outline" size={18} color="#FF6B35" />
              <Text style={[styles.guestsText, { writingDirection: 'rtl' }]}>{guests} {t.search.guests}</Text>
              <Ionicons name="chevron-down" size={16} color="#A0AEC0" style={{ marginRight: 'auto' }} />
            </TouchableOpacity>

            {/* Currency badge */}
            <View style={[styles.currencyRow, { flexDirection: 'row-reverse' }]}>
              <Ionicons name="cash-outline" size={15} color="#718096" />
              <Text style={[styles.currencyText, { writingDirection: 'rtl' }]}>{t.search.guests}: {currency.code} ({currency.label})</Text>
            </View>

            {/* Search Button */}
            <TouchableOpacity style={styles.searchBtn} onPress={handleSearch} activeOpacity={0.85}>
              <LinearGradient colors={['#FF6B35', '#e85520']} style={styles.searchBtnGrad} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
                <Text style={[styles.searchBtnText, { writingDirection: 'rtl' }]}>{t.search.search} الفنادق</Text>
        ...
        {guestModalVisible && (
          <View style={styles.guestModalOverlay}>
            ...
            <View style={styles.guestModal}>
              <Text style={[styles.guestModalTitle, { textAlign: 'right', writingDirection: 'rtl' }]}>عدد المسافرين</Text>
              {[1, 2, 3, 4, 5, 6].map(n => (
                <TouchableOpacity key={n} style={[styles.guestOption, { flexDirection: 'row-reverse' }, guests === n && styles.guestOptionActive]} onPress={() => { setGuests(n); setGuestModalVisible(false); }}>
                  <Ionicons name="person" size={16} color={guests === n ? '#FF6B35' : '#718096'} />
                  <Text style={[styles.guestOptionText, { writingDirection: 'rtl' }, guests === n && { color: '#FF6B35' }]}>{n} مسافر{n > 1 ? 'ون' : ''}</Text>
                  {guests === n && <Ionicons name="checkmark" size={16} color="#FF6B35" style={{ marginRight: 'auto' }} />}
                </TouchableOpacity>
              ))}
            </View>
          </View>
        ...
        <View style={styles.section}>
          <Text style={[styles.sectionTitle, { textAlign: 'right', writingDirection: 'rtl' }]}>{t.search.popularDestinations}</Text>
          <View style={[styles.destGrid, { flexDirection: 'row-reverse', flexWrap: 'wrap' }]}>
            {DESTINATIONS.map((dest, i) => (
              <TouchableOpacity key={i} style={styles.destCard} onPress={() => handleDestinationTap(dest)}>
                ...
                <LinearGradient colors={['transparent', 'rgba(0,0,0,0.7)']} style={styles.destOverlay}>
                  <Text style={[styles.destCity, { textAlign: 'right', writingDirection: 'rtl' }]}>{dest.city}</Text>
                  <Text style={[styles.destCountry, { textAlign: 'right', writingDirection: 'rtl' }]}>{dest.country}</Text>
                </LinearGradient>
              </TouchableOpacity>
            ))}
          </View>
        ...
        {featured.length > 0 && (
          <View style={styles.section}>
            <View style={[styles.sectionHeader, { flexDirection: 'row-reverse' }]}>
              <Text style={[styles.sectionTitle, { textAlign: 'right', writingDirection: 'rtl' }]}>{t.search.featured}</Text>
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.featuredCard} onPress={() => navigation.navigate('HotelDetail', { hotelId: item.id, checkIn, checkOut, guests })}>
                  ...
                  <LinearGradient colors={['transparent', 'rgba(0,0,0,0.75)']} style={styles.featuredOverlay}>
                    <Text style={[styles.featuredName, { textAlign: 'right', writingDirection: 'rtl' }]} numberOfLines={2}>{item.name}</Text>
                    <Text style={[styles.featuredCity, { textAlign: 'right', writingDirection: 'rtl' }]}>{item.city}</Text>
                    <View style={[styles.featuredPriceRow, { flexDirection: 'row-reverse' }]}>
                      <Text style={styles.featuredPrice}>من {item.bestOffer?.discountedPrice} {currency.label}</Text>
                      <View style={[styles.featuredStars, { flexDirection: 'row-reverse' }]}>
        ...
        {flashDeals.length > 0 && (
          <View style={styles.section}>
            <View style={[styles.sectionHeader, { flexDirection: 'row-reverse' }]}>
              <Animated.Text style={[styles.sectionTitle, { textAlign: 'right', writingDirection: 'rtl', transform: [{ scale: flashPulse }] }]}>⚡ {t.search.flashDeals}</Animated.Text>
            ...
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.flashCard} onPress={() => navigation.navigate('HotelDetail', { hotelId: item.hotelId })}>
                  ...
                  <View style={[styles.flashContent, { alignItems: 'flex-end' }]}>
                    <Text style={[styles.flashName, { textAlign: 'right', writingDirection: 'rtl' }]} numberOfLines={1}>{item.hotelName}</Text>
                    <View style={[styles.flashPriceRow, { flexDirection: 'row-reverse' }]}>
                      <Text style={styles.flashOrig}>{item.originalPrice} {currency.label}</Text>
                      <Text style={styles.flashNew}>{item.newPrice} {currency.label}</Text>
                    </View>
                  </View>
                </TouchableOpacity>
              )}
        ...
        {trending.length > 0 && (
          <View style={styles.section}>
            <View style={[styles.sectionHeader, { flexDirection: 'row-reverse' }]}>
              <Text style={[styles.sectionTitle, { textAlign: 'right', writingDirection: 'rtl' }]}>{t.search.trending} 🔥</Text>
              <TouchableOpacity onPress={() => navigation.navigate('HotelResults', { destination: '', checkIn, checkOut, guests })}>
                <Text style={styles.seeAll}>عرض الكل</Text>
              </TouchableOpacity>
            </View>
            <FlatList
              ...
              renderItem={({ item }) => (
                <TouchableOpacity style={styles.trendCard} onPress={() => navigation.navigate('HotelDetail', { hotelId: item.id, checkIn, checkOut, guests })}>
                  ...
                  {item.trendBadge && (
                    <View style={[styles.trendBadge, { right: 8, left: undefined }]}>
                      <Text style={[styles.trendBadgeText, { writingDirection: 'rtl' }]}>{item.trendBadge}</Text>
                    </View>
                  )}
                  <View style={[styles.trendContent, { alignItems: 'flex-end' }]}>
                    <Text style={[styles.trendName, { textAlign: 'right', writingDirection: 'rtl' }]} numberOfLines={1}>{item.name}</Text>
                    <Text style={[styles.trendCity, { textAlign: 'right', writingDirection: 'rtl' }]}>{item.city}</Text>
                    <Text style={styles.trendPrice}>من {item.bestPrice} {currency.label}</Text>
                  </View>
                </TouchableOpacity>
              )}
            />
        ...
        <TouchableOpacity style={styles.tripPlannerBtn} onPress={() => navigation.navigate('TripPlanner')}>
          <LinearGradient colors={['#004E89', '#1a6eac']} style={[styles.tripPlannerGrad, { flexDirection: 'row-reverse' }]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
            <Ionicons name="map-outline" size={22} color="#fff" />
            <View style={{ flex: 1, marginRight: 12, alignItems: 'flex-end' }}>
              <Text style={[styles.tripPlannerTitle, { writingDirection: 'rtl' }]}>تخطيط رحلة</Text>
              <Text style={[styles.tripPlannerSub, { writingDirection: 'rtl' }]}>وجهات متعددة · احسب ميزانيتك</Text>
            </View>
        ...
        <TouchableOpacity style={styles.promoBanner} onPress={() => navigation.navigate('HotelResults', { destination: 'Djerba', checkIn, checkOut, guests })}>
          <LinearGradient colors={['#004E89', '#1a6eac']} style={[styles.promoBannerGrad, { flexDirection: 'row-reverse' }]} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
            <View style={{ alignItems: 'flex-end' }}>
              <Text style={[styles.promoTitle, { writingDirection: 'rtl' }]}>🏖 عروض جربة</Text>
              <Text style={[styles.promoSub, { writingDirection: 'rtl' }]}>وفر حتى 30% هذا الصيف</Text>
            </View>
            <TouchableOpacity style={styles.promoBtn} onPress={() => navigation.navigate('HotelResults', { destination: 'Djerba', checkIn, checkOut, guests })}>
              <Text style={[styles.promoBtnText, { writingDirection: 'rtl' }]}>عرض العروض</Text>
            </TouchableOpacity>
          </LinearGradient>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  heroHeader: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 20 },
  ...
  inputGroup: { flexDirection: 'row-reverse', alignItems: 'center', backgroundColor: '#F7FAFC', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 10, marginBottom: 8, borderWidth: 1.5, borderColor: '#E2E8F0' },
  inputIconRTL: { marginLeft: 8 },
  ...
  suggestionItem: { flexDirection: 'row-reverse', alignItems: 'center', padding: 12, borderBottomWidth: 1, borderBottomColor: '#F7FAFC' },
  suggestionLabel: { fontSize: 14, fontWeight: '600', color: '#2D3748' },
  suggestionSub: { fontSize: 12, color: '#718096' },
  datesRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 8, marginBottom: 8 },
  ...
  dateControl: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between' },
  dateArrow: { padding: 2 },
  dateCenter: { flexDirection: 'row-reverse', alignItems: 'center', gap: 4 },
  ...
  guestsRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 8, backgroundColor: '#F7FAFC', borderRadius: 12, paddingHorizontal: 12, paddingVertical: 12, marginBottom: 8, borderWidth: 1.5, borderColor: '#E2E8F0' },
  guestsText: { fontSize: 14, color: '#2D3748', fontWeight: '500' },
  currencyRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6, paddingHorizontal: 4, marginBottom: 12 },
  currencyText: { fontSize: 12, color: '#718096' },
  searchBtn: { borderRadius: 14, overflow: 'hidden' },
  searchBtnGrad: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'center', gap: 8, paddingVertical: 15 },
  ...
  guestOption: { flexDirection: 'row-reverse', alignItems: 'center', gap: 10, paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: '#F7FAFC' },
  ...
  sectionHeader: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 16, marginBottom: 14 },
  ...
  destGrid: { flexDirection: 'row-reverse', flexWrap: 'wrap', paddingHorizontal: 10, gap: 6 },
  ...
  featuredPriceRow: { flexDirection: 'row-reverse', justifyContent: 'space-between', alignItems: 'center' },
  ...
  featuredStars: { flexDirection: 'row-reverse', gap: 1 },
  ...
  flashDiscBadge: { position: 'absolute', top: 10, right: 10, backgroundColor: '#E53E3E', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 4 },
  ...
  flashPriceRow: { flexDirection: 'row-reverse', alignItems: 'center', gap: 6 },
  ...
  trendBadge: { position: 'absolute', top: 10, right: 8, backgroundColor: 'rgba(0,0,0,0.6)', borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  tripPlannerGrad: { flexDirection: 'row-reverse', alignItems: 'center', padding: 16 },
  ...
  promoBannerGrad: { flexDirection: 'row-reverse', alignItems: 'center', justifyContent: 'space-between', padding: 20 },
