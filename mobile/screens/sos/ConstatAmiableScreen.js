      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ flexDirection: 'row', gap: 10, marginHorizontal: 0 }}>
          ...
          <TouchableOpacity
            ...
            onPress={async () => {
              try {
                const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/><style>body{font-family:Arial,sans-serif;padding:20px;background:#fff;color:#222}h1{color:#8E44AD;font-size:18px}table{width:100%;border-collapse:collapse;margin-top:10px}td,th{border:1px solid #ccc;padding:8px;font-size:12px}th{background:#f0f0f0}h2{font-size:14px;color:#555;margin-top:16px}</style></head><body><h1>⚖️ Constat Amiable EASYWAY</h1><p>Date : ${new Date().toLocaleDateString('fr-TN')}</p><h2>Véhicule A (Client)</h2><table><tr><th>Marque</th><td>${vehicleA.marque||''}</td><th>Modèle</th><td>${vehicleA.modele||''}</td></tr><tr><th>Immat.</th><td>${vehicleA.immatriculation||''}</td><th>Année</th><td>${vehicleA.annee||''}</td></tr><tr><th>Assureur</th><td>${vehicleA.assurance||''}</td><th>N° Police</th><td>${vehicleA.numPolice||''}</td></tr></table><h2>Véhicule B (Autre partie)</h2><table><tr><th>Marque</th><td>${vehicleB.marque||''}</td><th>Modèle</th><td>${vehicleB.modele||''}</td></tr><tr><th>Immat.</th><td>${vehicleB.immatriculation||''}</td><th>Année</th><td>${vehicleB.annee||''}</td></tr><tr><th>Assureur</th><td>${vehicleB.assurance||''}</td><th>N° Police</th><td>${vehicleB.numPolice||''}</td></tr></table><h2>Circonstances</h2><p>${selectedCircumstances.join(', ')||'Non renseignées'}</p><p style="margin-top:30px;font-size:10px;color:#999">Généré par EASYWAY · ${new Date().toISOString()}</p></body></html>`;
