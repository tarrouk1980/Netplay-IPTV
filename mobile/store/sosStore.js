...
const useSosStore = create((set, get) => ({
  ...
  requestSOS: async (data) => {
    ...
    try {
      const response = await api.post('/api/sos/request', data);
  ...
  submitQuote: async (orderId, price, eta) => {
    const response = await api.post(`/api/sos/${orderId}/quote`, {
      price,
  ...
  acceptQuote: async (orderId, depanneurId) => {
    const response = await api.post(`/api/sos/${orderId}/accept-quote`, { depanneurId });
    const { order } = response.data;
  ...
  confirmArrival: async (orderId) => {
    const response = await api.post(`/api/sos/${orderId}/start`);
    const { order } = response.data;
    set({ currentSOSOrder: order });
    return order;
  },
  ...
  completeRide: async (orderId) => {
    const response = await api.post(`/api/sos/${orderId}/complete`);
  ...
  cancelSOS: async (orderId, reason) => {
    const response = await api.post(`/api/sos/${orderId}/cancel`, { reason });
    const { order } = response.data;
    set({ currentSOSOrder: order });
    return order;
  },
  ...
  fetchNearbyRequests: async () => {
    const response = await api.get('/api/sos/depanneur/requests');
  ...
  fetchOrder: async (orderId) => {
    const response = await api.get(`/api/sos/${orderId}`);
    const { order } = response.data;
  ...
  submitCounterOffer: async (orderId, counterPrice) => {
    const response = await api.post(`/api/sos/${orderId}/counter-offer`, { counterPrice });
    const { order } = response.data;
    if (order) set({ currentSOSOrder: order });
    return response.data;
  },
  ...
  acceptCounterOffer: async (orderId) => {
    const response = await api.post(`/api/sos/${orderId}/accept-counter`);
    const { order } = response.data;
    if (order) set({ currentSOSOrder: order });
    return response.data;
  },
