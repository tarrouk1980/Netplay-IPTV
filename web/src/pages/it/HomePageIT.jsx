...
const DESTINATIONS = [
  { city: 'Marrakech', country: 'Marocco', flag: '🇲🇦', price: 45, gradient: 'linear-gradient(135deg, #764ba2 0%, #667eea 100%)' },
  { city: 'Casablanca', country: 'Marocco', flag: '🇲🇦', price: 38, gradient: 'linear-gradient(135deg, #004E89 0%, #1a6eac 100%)' },
  { city: 'Tunisi', country: 'Tunisia', flag: '🇹🇳', price: 35, gradient: 'linear-gradient(135deg, #e74c3c 0%, #c0392b 100%)' },
  { city: 'Djerba', country: 'Tunisia', flag: '🇹🇳', price: 40, gradient: 'linear-gradient(135deg, #4facfe 0%, #00f2fe 100%)' },
]

const FLIGHT_TIMES = [
  { route: 'Roma → Tunisi', time: '1h50', icon: '🇹🇳' },
  { route: 'Milano → Marrakech', time: '2h40', icon: '🇲🇦' },
  { route: 'Roma → Casablanca', time: '3h10', icon: '🇲🇦' },
  { route: 'Milano → Djerba', time: '2h00', icon: '🇹🇳' },
]
...
export default function HomePageIT() {
  ...
  return (
    <div style={{ background: '#F8F9FA' }}>
      <GDPRBanner />
      <title>EasyHotels — Confronta gli hotel in Marocco e Tunisia | Prezzi migliori</title>
      ...
      <section style={{
        ...
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 900, width: '100%' }}>
          <div style={{
            ...
            ✈ Confronta 500+ hotel — I prezzi migliori garantiti
          </div>

          <h1 style={{ fontSize: 'clamp(28px, 5.5vw, 58px)', fontWeight: 900, color: '#fff', lineHeight: 1.15, marginBottom: 20 }}>
            Confronta gli hotel in{' '}
            <span style={{ color: '#FFC72C' }}>Marocco e Tunisia</span>
          </h1>

          <p style={{ fontSize: 18, color: 'rgba(255,255,255,0.9)', marginBottom: 40, lineHeight: 1.6 }}>
            Booking.com, Expedia, Hotels.com — tutto in un unico posto. Risparmia fino al 60%.
          </p>
          ...
          <div style={{ background: 'rgba(255,255,255,0.97)', borderRadius: 16, padding: 24, boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            <form onSubmit={handleSearch} style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
              <input
                type="text"
                placeholder="Destinazione, hotel..."
                value={destination}
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 16, marginTop: 28 }}>
            {[
              { icon: '💶', label: 'Pagamento in €' },
              { icon: '🇮🇹', label: 'Supporto in italiano' },
              { icon: '🔍', label: '50+ fonti' },
              { icon: '✅', label: 'Senza costi nascosti' },
            ].map((b, i) => (
      ...
      {/* DIASPORA BANNER */}
      <section style={{ background: 'linear-gradient(135deg, #c0392b 0%, #e74c3c 100%)', padding: '40px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 20 }}>
          <div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
              ...
              <span style={{ fontSize: 28 }}>🇹🇳</span>
              <h2 style={{ fontSize: 22, fontWeight: 800, color: '#fff', marginLeft: 4 }}>La comunità maghrebina in Italia</h2>
            </div>
            <p style={{ color: 'rgba(255,255,255,0.9)', fontSize: 16, maxWidth: 520 }}>
              Torni in Marocco quest'estate? Trova il miglior hotel e confronta i prezzi in pochi secondi.
            </p>
          </div>
          <button
            onClick={() => navigate('/search?destination=Marocco')}
            style={{ background: '#fff', color: '#c0392b', padding: '14px 28px', borderRadius: 10, fontSize: 15, fontWeight: 700, border: 'none', cursor: 'pointer', flexShrink: 0 }}
          >
            Vedi hotel in Marocco →
          </button>
        </div>
      </section>
      ...
      <section style={{ background: '#fff', padding: '50px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <h2 style={{ textAlign: 'center', fontSize: 26, fontWeight: 800, color: '#2d3748', marginBottom: 8 }}>✈️ Voli da Roma e Milano</h2>
          <p style={{ textAlign: 'center', color: '#718096', marginBottom: 36 }}>Destinazioni vicine, a meno di 3 ore di volo</p>
      ...
      <section style={{ padding: '80px 24px', maxWidth: 1100, margin: '0 auto' }}>
        <div style={{ textAlign: 'center', marginBottom: 48 }}>
          <h2 style={{ fontSize: 34, fontWeight: 800, color: '#2d3748', marginBottom: 12 }}>🌍 Destinazioni popolari</h2>
          <p style={{ color: '#718096', fontSize: 16 }}>Le città più ricercate dalla comunità italiana</p>
        </div>
              ...
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  ...
                  <div style={{ textAlign: 'right' }}>
                    <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: 11 }}>da</div>
                    <div style={{ color: '#FFC72C', fontSize: 18, fontWeight: 800 }}>{d.price}€<span style={{ fontSize: 12, fontWeight: 400 }}>/notte</span></div>
                  </div>
                </div>
              </div>
            </div>
      ...
      <section style={{ padding: '60px 24px', background: '#f0f4f8' }}>
        <div style={{ maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
          <h2 style={{ fontSize: 28, fontWeight: 800, color: '#2d3748', marginBottom: 12 }}>Hotel che rispettano i tuoi valori</h2>
          <p style={{ color: '#718096', fontSize: 15, marginBottom: 40 }}>Filtra gli hotel secondo le tue esigenze e convinzioni</p>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 14 }}>
            {[
              { key: 'halal', icon: '🍖', label: 'Halal certificato' },
              { key: 'noAlcohol', icon: '🚫', label: 'Senza alcol' },
              { key: 'burkini', icon: '🧕', label: 'Burkini OK' },
              { key: 'family', icon: '👨‍👩‍👧', label: 'Ideale famiglie' },
            ].map(f => (
              <div
                key={f.key}
      ...
      <section style={{ background: 'linear-gradient(135deg, #004E89 0%, #1a6eac 100%)', padding: '60px 24px', textAlign: 'center' }}>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          ...
          <h2 style={{ fontSize: 30, fontWeight: 800, color: '#fff', marginBottom: 12 }}>Sei un albergatore?</h2>
          <p style={{ color: 'rgba(255,255,255,0.85)', fontSize: 17, marginBottom: 32 }}>
            Unisciti a <strong>47 partner EasyHotels</strong> e aumenta la tua visibilità
          </p>
